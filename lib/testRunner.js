...
class TestRunner {
  ...
  setupPaths(testID) {
    ...
    fs.mkdirSync(this.paths['results'], { recursive: true });
  ...
  async createFilmStrip() {
    ...
    try {
      ...
      const filmstripFiles = await process.then(
        function (video) {
      ...
      this.resultAssets.filmstripFiles = filmstripFiles.map(filePath =>
        path.relative(this.paths['results'], filePath),
      );
    } catch (e) {
  ...
  cleanup() {
    log('Cleanup started');
    fs.rmSync(this.paths['temporaryContext'], { recursive: true, force: true });
    log('Cleanup ended');
